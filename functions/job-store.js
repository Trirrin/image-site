...
export class JobStore {
  ...
  async fetch(request) {
    ...
    try {
      if (request.method === 'PUT' && url.pathname === '/jobs') return await this.putJob(request)
      if (request.method === 'GET' && url.pathname === '/jobs') return await this.listJobs()
      const payloadMatch = url.pathname.match(/^\/jobs\/([^/]+)\/payload$/)
      if (request.method === 'PUT' && payloadMatch) return await this.putPayload(payloadMatch[1], request)
      if (request.method === 'GET' && payloadMatch) return await this.getPayload(payloadMatch[1])
      const imagesMatch = url.pathname.match(/^\/jobs\/([^/]+)\/images$/)
      if (request.method === 'PUT' && imagesMatch) return await this.putImages(imagesMatch[1], request)
      const imageMatch = url.pathname.match(/^\/jobs\/([^/]+)\/images\/([^/]+)$/)
      if (request.method === 'GET' && imageMatch) return await this.getImage(imageMatch[1], imageMatch[2])
      if (request.method === 'GET' && url.pathname.startsWith('/jobs/')) return await this.getJob(url.pathname.slice('/jobs/'.length))
      if (request.method === 'DELETE' && url.pathname === '/expired') return await this.deleteExpired()
